...
class Product( models.Model ):
	...
	def save( self, *args, **kwargs ):
		...
		if self.image_one.name and len(self.image_one.name) > 0:
			...
			file1name_org = one_original[0]
			one_ext = one_original[1]
			one_new = '{0}-{1}-{2}.{3}'.format( vendor, productName, file1name_org[:2], one_ext )
		...
		if self.image_two.name and len(self.image_two.name) > 0:
			...
			file2name_org = two_original[0]
			two_ext = two_original[1]
			two_new = '{0}-{1}-{2}.{3}'.format( vendor, productName, file2name_org[:2], two_ext )
		...
		if self.image_three.name and len(self.image_three.name) > 0:
			...
			file3name_org = three_original[0]
			three_ext = three_original[1]
			three_new = '{0}-{1}-{2}.{3}'.format( vendor, productName,file3name_org[:2], three_ext )
